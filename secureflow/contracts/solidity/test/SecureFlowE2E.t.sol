 ...
 * more or less.
 */
contract SecureFlowE2ETest is JobManagerBase {
    /* ─────────────────────── helpers ─────────────────────── */
     ...
     * SecureFlow exactly as it works today, walked end to end.
     */
    function test_e2e_manualJob_postToPaidAndRated() public {
        ...
        SecureFlow.Escrow memory esc = sf.getEscrow(id);
        assertEq(uint8(esc.status), uint8(SecureFlow.EscrowStatus.Released), "escrow released");
    ...
    function test_e2e_autopilotJob_withRejectionAndRevision() public {
        ...
        SecureFlow.Escrow memory esc = sf.getEscrow(id);
        assertEq(uint8(esc.status), uint8(SecureFlow.EscrowStatus.Released));
    ...
    function test_e2e_autopilotEscalatesToArbiter() public {
        ...
        vm.prank(manager);
        vm.expectRevert(SecureFlow.EscrowNotActive.selector);
        sf.approveMilestone(id, 1);
    ...
    function test_e2e_unfilledJobCancelledAndRefunded() public {
        ...
        SecureFlow.Escrow memory esc = sf.getEscrow(id);
        assertEq(uint8(esc.status), uint8(SecureFlow.EscrowStatus.Cancelled));
