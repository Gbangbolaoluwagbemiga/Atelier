...
contract YieldHandler is Test {
    ...
    constructor(
        SecureFlow _sf, MockUSDC _usdc, MockYieldAdapter _venue,
...
contract ProductiveEscrowInvariantTest is JobManagerBase {
     ...
     * SecureFlow.sol rather than hidden behind the word "always".
     */
    ...
    function invariant_deployedNeverExceedsWhatIsHeld() public view {
        SecureFlow.Escrow memory esc = sf.getEscrow(jobId);
        if (
            esc.status != SecureFlow.EscrowStatus.Pending &&
            esc.status != SecureFlow.EscrowStatus.InProgress
        ) return;
    ...
    function test_healthyVenueKeepsTheNextClaimInCash() public {
        ...
        SecureFlow.Milestone[] memory ms = sf.getMilestones(jobId);
        uint256 largest;
        for (uint256 i; i < ms.length; ++i) {
            if (ms[i].status == SecureFlow.MilestoneStatus.Approved) continue;
