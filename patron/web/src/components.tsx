...
import { displayStatus, milestoneStates, parseBrief, type AgentEvent, type DecisionRow, type MilestoneState, type PaymentRow, type TaskRow } from "./types";
...
export function PostQuest({ onPosted }: { onPosted: () => void }) {
  ...
  const [window_, setWindow] = useState<"3" | "30" | "120" | "1440">("30");
  ...
  async function submit() {
    ...
    const WINDOW_PHRASE: Record<string, string> = {
      "3": " Give people 3 minutes to apply before choosing.",
      ...
      <div className="post-quest-fields">
        ...
        <label className="post-quest-field">
          ...
          <select value={window_} onChange={(e) => setWindow(e.target.value as typeof window_)}>
            <option value="3">3 minutes (demo)</option>
  ...
  const inner = (
    <>
      <div className="card-row">
        ...
        {(() => {
          const d = displayStatus(task);
          return <span className={`badge status-${d.tone}`}>{d.label}</span>;
        })()}
      </div>
